...
AnimatedSpriteType::~AnimatedSpriteType() 
{
	map<wstring, vector<unsigned int>*>::iterator animationsIt = animationSequences.begin();
	while (animationsIt != animationSequences.end())
	{
		map<wstring, vector<unsigned int>*>::iterator tempIt = animationsIt;
		animationsIt++;
		vector<unsigned int> *animationToDelete = tempIt->second;
