...
SparseLayer::~SparseLayer()
{
	// GO THROUGH AND DELETE ALL HEAP OBJECTS
	vector<OverlayImage*>::iterator it = sparseTiles->begin();
	while (it != sparseTiles->end())
	{
		vector<OverlayImage*>::iterator tempIt = it;
		it++;
		OverlayImage *overlayImageToDelete = (*tempIt);
		delete overlayImageToDelete;
	}
	sparseTiles->clear();
	delete sparseTiles;
}
