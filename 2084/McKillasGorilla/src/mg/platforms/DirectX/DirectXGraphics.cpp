...
HRESULT DirectXGraphics::createDirectXDeviceAndSpriteHandler()
{
	...
	if (SUCCEEDED(result))
	{
		text->writeDebugOutput(L"SUCCEEDED");

		text->writeDebugOutput(L"\nD3DXCreateSprite(): ");
		...
		if (SUCCEEDED(result))
		{
			...
			text->writeDebugOutput(L"FAILED");
	}
	else
		text->writeDebugOutput(L"FAILED");
...
void DirectXGraphics::endDirectXFrameRendering()
{
    ...
    if (FAILED(graphicsDevice->EndScene()))
		game->getText()->writeDebugOutput(L"\ngraphicsDevice->EndScene(): FAILED");
...
/*
	startUp - This is the entry point for the application setting up the
...
*/
void DirectXGraphics::startUpPlatform()
{
	Game *game = Game::getSingleton();
	...
	wstring isFullscreenProp = importer->getProperty(props.PROP_NAME_IS_FULLSCREEN);
	boolean isFullscreen = isFullscreenProp.compare(props.PROP_VALUE_TRUE) == 0;
	...
	GameOS *os = Game::getSingleton()->getOS();
...
/*
	...
	are using.
*/
void DirectXGraphics::setDebugFont(int fontSize)
{
...
{
	// GET THE MATRIX USED FOR RENDERING GUI CONTROLS
	D3DXMATRIX guiMatrix;
	spriteHandler->GetTransform(&guiMatrix);

	// WE'LL USE THIS FOR POSITIONING AND ROTATING SPRITES
	...
	RECT rect;
	D3DXVECTOR2 position = D3DXVECTOR2();
	D3DXVECTOR2 center = D3DXVECTOR2();
	sort(worldRenderList->begin(), worldRenderList->end(), RenderItem());

	// GO THROUGH EACH ITEM IN THE LIST
	bool newText;
	int id = 0;
	if (worldRenderList->hasNext()) {
	...
	while (worldRenderList->hasNext())
	{
		...
		if (id >= 0)
		{
			...
			position.x = itemToRender.x + viewport->getViewportOffsetX();
			position.y = itemToRender.y + viewport->getViewportOffsetY();
			center.x = (itemToRender.width / 2);
			center.y = (itemToRender.height / 2);
			
			rect.left = 0;
			rect.right = itemToRender.width;
			rect.top = 0;
			rect.bottom = itemToRender.height;
			D3DXMatrixAffineTransformation2D(&spriteMatrix, 1.0f, &center, itemToRender.rotationInRadians, &position);
			spriteHandler->SetTransform(&spriteMatrix);
	...
	// RESTORE THE GUI RENDERING MATRIX
	spriteHandler->SetTransform(&guiMatrix);
}


void DirectXGraphics::renderGame()
{
	Game *game = Game::getSingleton();
...
*/
void DirectXGraphics::renderText(RenderText *textToDraw)
{
	Game *game = Game::getSingleton();
...
*/
void DirectXGraphics::shutDownPlatform()
{
	if (textFont)
	{
		textFont->Release();
