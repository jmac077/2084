...
*/
bool PoseurSpriteTypeImporter::load(wstring spriteTypesFileDir, wstring spriteTypesFile)
{
	Game *game = Game::getSingleton();

	int slashIndex = spriteTypesFile.rfind('/');
	dir = spriteTypesFileDir;
	const char *spriteTypesListFile = xmlReader.newCharArrayFromWstring(dir + spriteTypesFile);
	...
	for (unsigned int i = 0; i < spriteTypes.size(); i++)
	{
		...
		map<unsigned int, unsigned int> realImageIds;
		for (unsigned int j = 0; j < images.size(); j++)
		{
			string imageToLoad = images[j];
			wstring wImageToLoad(imageToLoad.begin(), imageToLoad.end());
			unsigned realId = tm->loadTexture(wImageToLoad);
			realImageIds[j] = realId;
		}

		wstring wSpriteType(spriteType.begin(), spriteType.end());
		AnimatedSpriteType *ast = new AnimatedSpriteType();
		unsigned int spriteTypeId = game->getGSM()->getSpriteManager()->addSpriteType(wSpriteType, ast);
		...
		while (it != animations.end())
		{
			...
			{
				Pose pose = *poseIt;
				// LOOK UP THE REAL IMAGE ID
				unsigned int realId = realImageIds[pose.imageId];
				ast->addAnimationFrame(wKey, realId, pose.duration);
				poseIt++;
			}				
			it++;
		}
	...
	// AND NOW CLEAN UP
	spriteTypes.clear();
	spriteTypesDimensions.clear();
	spriteTypesImages.clear();
	spriteTypesAnimationsLists.clear();
...
{
	string fullPath = string(dir.begin(), dir.end()) + spriteTypeName;
	string spriteTypeFile = spriteTypeName + XML_FILE_EXTENSION;
	string spriteTypePath = fullPath + "/" + spriteTypeFile;
	const char *path = xmlReader.newCharArrayFromString(spriteTypePath);
														...
														string state)
{
	...
	while (poseNode)
	{
		...
		Pose poseToAdd;
		poseToAdd.imageId = xmlReader.extractIntAtt(poseElement, IMAGE_ID_ATT);
		poseToAdd.duration = xmlReader.extractIntAtt(poseElement, DURATION_ATT);
...
bool PoseurSpriteTypeImporter::loadImagesListInfo(	const TiXmlNode *node, const string fullPath, const string spriteTypeName)
{
	...
	while (imageNode)
	{
		...
		int id = xmlReader.extractIntAtt(imageElement, ID_ATT);
		string fileName = xmlReader.extractCharAtt(imageElement, FILE_NAME_ATT);
...
bool PoseurSpriteTypeImporter::unload()
{
	return false;
}