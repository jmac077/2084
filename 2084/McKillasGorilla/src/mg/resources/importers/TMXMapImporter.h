...
class TMXMapImporter : public MGImporter
{
protected:
...
public:
	TMXMapImporter() {}
	~TMXMapImporter(){}
	bool load(wstring mapFileDir, wstring mapFile);
	bool unload();

	// THESE ARE ALL THE HELPER METHODS
private:
	bool buildWorldFromInfo();
	void buildTiledLayer(TiledLayerInfo *tli, int idOffset, int z);
	void buildImageLayer(ImageLayerInfo *ili, int idOffset);
	void buildSparseLayer(SparseLayerInfo *sli, int idOffset);
