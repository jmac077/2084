...
#include "mg\game\IllegalArgumentException.h"
...
#include "mg\text\TextFileReader.h"
...
void GameGUI::startUp()
{
	...
	wstring offsetX = importer->getProperty(props.PROP_NAME_VIEWPORT_OFFSET_X);
	int viewportOffsetX;
	wstringstream(offsetX) >> viewportOffsetX;
	viewport.setViewportOffsetX(viewportOffsetX);
	wstring offsetY = importer->getProperty(props.PROP_NAME_VIEWPORT_OFFSET_Y);
	int viewportOffsetY;
	wstringstream(offsetY) >> viewportOffsetY;
...
*/
void GameGUI::addRenderItemsToRenderList()
{
	Game *game = Game::getSingleton();
...
*/
bool GameGUI::checkCurrentScreenForAction()
{
	Game *game = Game::getSingleton();
